...
function RevealMysteryItem(items, result, char)
	...
	char:getInventory():AddItem("PynoTweaks.Bong" .. colors[roll + 1]);
end
...
function RevealSuspectItem(items, result, char)
	...
	char:getInventory():AddItem("PynoTweaks." .. illicit[roll + 1]);
end
	...
	for i=0, (items:size() - 1) do
		if string.sub(items:get(i):getType(), 5, 5) == "_" then
			...
			if string.sub(result:getType(), 1, 4) == "Toba" then
				char:getInventory():AddItem("PynoTweaks.TobaccoBong" .. string.sub(items:get(i):getType(), 5));
			elseif string.sub(result:getType(), 1, 4) == "Shak" then
				char:getInventory():AddItem("PynoTweaks.ShakeBong" .. string.sub(items:get(i):getType(), 5));
			else
				char:getInventory():AddItem("PynoTweaks." .. string.sub(result:getType(), 1, 4) .. "Bong" .. string.sub(items:get(i):getType(), 5));
			end
		end
	end
	if not colored then
		if string.sub(result:getType(), 1, 4) == "Toba" then
			char:getInventory():AddItem("PynoTweaks.TobaccoBong");
		elseif string.sub(result:getType(), 1, 4) == "Shak" then
			char:getInventory():AddItem("PynoTweaks.ShakeBong");
		else
			char:getInventory():AddItem("PynoTweaks." .. string.sub(result:getType(), 1, 4) .. "Bong");
		end
	end
end
	...
	for i=0, (items:size() - 1) do
		if items:get(i):getType() == "GFGrinder" then
			...
			if items:get(i):getModData().kiefamt >= 0.05 then
				...
				char:getInventory():AddItem("PynoTweaks.Kief"):setUsedDelta(0.05);
...
function Create_Cigs(items, result, player)
	...
	for i=1, times do
		player:getInventory():AddItem("PynoTweaks.GFCigarettes");
	end
end
...
function GF_GiveGFCigs(items, result, player)
	local times = 20;
	for i=1, times do
		player:getInventory():AddItem("PynoTweaks.GFCigarette");
	end
end

function BreakBlunt(items, result, char)
	char:getInventory():AddItem("PynoTweaks.Tobacco");
end

function OpenBlunt(items, result, char)
	char:getInventory():AddItem("PynoTweaks.BluntWrap");
end

--Returns the SmokingPipe after dumping its contents
function DumpPipe_OnCreate(items, result, player)

	player:getInventory():AddItem("PynoTweaks.SmokingPipe");
...
function DumpW_OnCreate(items, result, player)
	...
	for i=0,(items:size() - 1) do
		...
		elseif items:get(i):getModData().budtype == "Cannabis" or items:get(i):getModData().budtype == nil then
			if ProceduralDistributions == nil then
			player:getInventory():AddItem("PynoTweaks.Cannabis");
			else
			player:getInventory():AddItem("PynoTweaks.Cannabis", 1);
			end
		end
		if string.sub(items:get(i):getType(), 9, 9) == "_" then
			colored = true;
			player:getInventory():AddItem("PynoTweaks.Bong" .. string.sub(items:get(i):getType(), 9));
		end
	end
	if not colored then
		player:getInventory():AddItem("PynoTweaks.Bong");
	end
end
...
function DumpBong_OnCreate(items, result, player)
	...
	for i=0, (items:size() - 1) do
		...
		elseif string.sub(items:get(i):getType(), 10, 10) == "_" then
			colored = true;
			player:getInventory():AddItem("PynoTweaks.Bong" .. string.sub(items:get(i):getType(), 10));
		elseif string.sub(items:get(i):getType(), 12, 12) == "_" then
			colored = true;
			player:getInventory():AddItem("PynoTweaks.Bong" .. string.sub(items:get(i):getType(), 12));
		end
	end
	if not colored then
		player:getInventory():AddItem("PynoTweaks.Bong");
